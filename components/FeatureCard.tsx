...
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description, onClick }) => {
  return (
    <div 
      onClick={onClick}
      className="group bg-white p-4 sm:p-8 rounded-2xl sm:rounded-[2rem] shadow-sm hover:shadow-2xl hover:shadow-emerald-500/10 transform hover:-translate-y-2 transition-all duration-500 cursor-pointer flex flex-col items-center text-center border border-slate-100 relative overflow-hidden"
    >
      ...
      <h3 className="text-sm sm:text-xl font-black mb-2 sm:mb-3 text-slate-800 group-hover:text-emerald-700 transition-colors relative z-10 leading-tight">{title}</h3>
      <p className="text-slate-500 font-medium leading-relaxed relative z-10 text-xs sm:text-sm hidden sm:block">{description}</p>
